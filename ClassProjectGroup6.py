...
def vegasLongestAccidents(data_frame):
    ...
    print("\n------------------------------------------------------------------")
    print(f"[{current_time}] 10. What was the longest accident (in hours) " +
          "recorded in Las Vegas in" +
          " the Spring (March, April, and May)?")
    print(f"[{current_time}]Displaying the longest accidents of each month" + 
          " in Spring per year:")
    ...
    for year in range(min_year, (max_year + 1)):
        print(f"\n\n[{current_time}] The longest accident recorded in Las Vegas" + 
              " in Spring of", year, "was :")
        ...
        if not march.empty:
            ...
            print(f"March: {max_time_march['Time_in_Hours']: .3f} hours")
        else:
        ...
        if not april.empty:
            ...
            print(f"April: {max_time_april['Time_in_Hours']: .3f} hours")
        else:
        ...
        if not may.empty:
            ...
            print(f"May: {max_time_may['Time_in_Hours']: .3f} hours")
        else:
...
def checkState(state):
    try:
        if len(state) != 2 or not state.isalpha():
            raise ValueError("Invalid state input. Must be state's abbreviation.")
...
def checkCity(city):
    try:
        if not city.isalpha():
            raise ValueError("Invalid city input. Only letters can be entered for" + 
                             " the city.")
    except ValueError as err:
        #print error message
        print(err)
...
def checkZipcode(zipcode):
    try:
        ...
        if (len(zipcode) != 5) or not zipcode.isdigit():
            raise ValueError("Invalid Zipcode. Must be a 5-digit Zipcode.")
    except ValueError as err:
        #print error message
        print(err)
        return False
...
def searchStateCityZip(df):
    ...
    if state and city and zipcode:
        ...
        if check1 and check2 and check3:
            ...
            end_time = time.time()
            total_time = end_time - start_time
            print(f"\nTime to perform search is: {total_time: .2f} seconds")
            return
    elif state and city:
        check1 = checkState(state)
        check2 = checkCity(city)
...
def checkYear(year):
    try:
        if not year.isdigit() or (len(year) != 4):
            raise ValueError("Invalid input for year. Year must be 4 digits.")
    except ValueError as err:
        print(err)
        return False
...
def checkMonth(month):
    try:
        if (int(month) < 1) or (int(month) > 12):
            raise ValueError("Invalid input for month. Value must be between 1-12.")
    except ValueError as err:
        print(err)
        return False
...
def checkDay(day):
    try:
        if (int(day) < 1) or (int(day) > 31):
            raise ValueError("Invalid input for day. Value must be between 1-31.")
    except ValueError as err:
        print(err)
        return False
